...
class Task {
        ...
        tasks.forEach((task, index) => {
            _tasks += `  
                ...
                <div class="task-item ${task.state === "show" ? 'mt-2 d-flex justify-content-between align-items-center' : 'd-none'}">
                    ...
                    <button class="bg-success "> <input type="checkbox" class="btn btn-sm btn-success is__completed " id="complete" name="complete" value="yes" onclick="Task.todoCompleted('${task.id}')"><i class=""></i></input></button>
    ...
    static update(task) {
        ...
        tasks.forEach((item, index) => {
            if (`${item.id}` === task) {
                ...
                edit[index].innerHTML = `
                    <input type="text" id="task-input" class="form-control" value="${item.name}" placeholder="Edit this Todo and  Enter your Todo!" title="Edit this Todo " />
                    <button type="button" class="btn btn-primary save-edited-todo">Update</button>
                `;
        ...
        showMsg('.msg','Successfully Updated');
        this.display();
    }
    ...
    static delete(task) {
        ...
        //     }
        // });
        showMsg('.msg','Successfully Deleted ');
        this.display();
    }
...
createBtn.addEventListener('click', (e) => {
    ...
    if (input_value !== '') {
        ...
        showMsg('.msg','Successfully added new Todo');
        
    } else { 
        showError('.error', 'Cannot Add Todo! Type your task /Todo ...');
    }
});
...
// show alert
function showMsg(show_place, show_message) {
    const show_container = document.querySelector(show_place);
    if (show_container) {
        ...
        `;
        setTimeout(() => show_container.innerHTML = '', 3000);
    }
}
